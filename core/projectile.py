...
_projectile_sprite = None

def _get_projectile_sprite():
    global _projectile_sprite
    if _projectile_sprite is None:
        _projectile_sprite = pg.image.load("assets/textures/sprjatovi-png/Projektil.png").convert_alpha()
    return _projectile_sprite
...
class Projectile:
    ...
    def draw(self, screen, mode7):
        ...
        if scale > 0:
            radius = max(8, scale // self.radius_divisor)
            size = radius * 2
            sprite = _get_projectile_sprite()
            scaled = pg.transform.scale(sprite, (size, size))
            screen.blit(scaled, (int(screen_x) - radius, int(screen_y) - radius))
